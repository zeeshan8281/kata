...
<style>
:root{--bg:#0a0a0a;--fg:#fafafa;--muted:#a3a3a3;--card:#141414;--card2:#171717;
--border:#ffffff14;--primary:#1a0c6d;--accent:#8b7bff;--good:#4ade80}
...
.brand .dot{width:10px;height:10px;border-radius:2px;background:linear-gradient(135deg,var(--primary),var(--accent))}
.nav-r{display:flex;align-items:center;gap:14px}
.nav-r a.ghost{color:var(--muted);font-size:.9rem}
.btn{border:1px solid var(--border);background:#fff;color:#0a0a0a;font-weight:600;font-size:.88rem;
border-radius:8px;padding:8px 14px;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;gap:8px}
.btn:hover{opacity:.9}
.btn.gh{background:#fff}
.btn.outline{background:transparent;color:var(--fg)}
.avatar{width:26px;height:26px;border-radius:50%;border:1px solid var(--border)}
.usr{display:flex;align-items:center;gap:8px;font-size:.88rem;font-weight:600}
.usr .signout{color:var(--muted);font-weight:400;font-size:.8rem;cursor:pointer}
/* hero */
.hero{padding:72px 0 48px;text-align:center}
.hero .bar{height:4px;width:56px;background:linear-gradient(90deg,var(--primary),var(--accent));border-radius:9999px;margin:0 auto 26px}
.hero h1{font-size:3.4rem;line-height:1.05;letter-spacing:-1.8px;font-weight:700;margin:0 0 16px}
.hero h1 .g{background:linear-gradient(120deg,#fff,var(--accent));-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}
.hero p{font-size:1.18rem;color:var(--muted);max-width:640px;margin:0 auto 12px}
.hero .tags{color:var(--muted);font-family:'Geist Mono',monospace;font-size:.85rem;margin-top:18px}
.hero .cta{display:flex;gap:12px;justify-content:center;margin-top:30px;flex-wrap:wrap}
.hero .btn{padding:11px 20px;font-size:.95rem}
.eyebrow{font-family:'Geist Mono',monospace;font-size:.78rem;letter-spacing:.12em;text-transform:uppercase;color:var(--accent);margin-bottom:10px}
/* sections */
section{padding:44px 0;border-top:1px solid var(--border)}
h2{font-size:1.9rem;letter-spacing:-.6px;margin:0 0 6px}
.sub{color:var(--muted);margin:0 0 26px}
/* steps */
.steps{display:grid;gap:14px}
.step{display:flex;gap:16px;background:var(--card);border:1px solid var(--border);border-radius:12px;padding:18px 20px}
.step .n{flex:0 0 30px;height:30px;border-radius:8px;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-family:'Geist Mono',monospace}
.step h3{margin:2px 0 6px;font-size:1.05rem}
.step p{margin:0;color:var(--muted);font-size:.95rem}
pre{background:#000;border:1px solid var(--border);border-radius:10px;padding:14px 16px;overflow:auto;margin:10px 0 0;
...
.copy{background:transparent;border:1px solid var(--border);color:var(--muted);border-radius:7px;padding:4px 10px;font-size:.76rem;cursor:pointer;font-family:'Geist Mono',monospace}
.copy:hover{color:var(--fg)}
/* katas */
.kgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px}
.kcard{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:18px;display:flex;flex-direction:column;gap:8px}
...
.kcard h3{margin:0;font-size:1.1rem;font-family:'Geist Mono',monospace}
.kcard p{margin:0;color:var(--muted);font-size:.9rem}
.bite{font-family:'Geist Mono',monospace;letter-spacing:1px}
.bite .on{color:var(--accent)}.bite .off{color:#333}
/* scoring */
.formula{background:#000;border:1px solid var(--border);border-radius:12px;padding:22px;text-align:center;
font-family:'Geist Mono',monospace;font-size:1.05rem}
.fx{display:flex;gap:14px;flex-wrap:wrap;margin-top:16px}
.fx div{flex:1;min-width:200px;background:var(--card);border:1px solid var(--border);border-radius:10px;padding:14px 16px;font-size:.9rem;color:var(--muted)}
.fx b{color:var(--fg)}
/* leaderboard */
.kata-sub{color:var(--muted);font-family:'Geist Mono',monospace;font-size:.8rem;font-weight:400}
.card{background:var(--card2);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-top:12px}
...
.chip{display:inline-block;background:var(--primary);color:#fff;border-radius:9999px;padding:2px 10px;font-size:.72rem;font-family:'Geist Mono',monospace}
...
.lb-h h3{margin:0;font-size:1.2rem}
/* signin modal */
...
.sheet{background:#141414;border:1px solid var(--border);border-radius:14px;padding:26px;width:min(400px,92vw)}
.sheet h3{margin:0 0 6px;font-size:1.25rem}
.sheet p{margin:0 0 16px;color:var(--muted);font-size:.9rem}
.sheet input{width:100%;background:#000;border:1px solid var(--border);color:var(--fg);border-radius:9px;padding:11px 13px;font-family:'Geist Mono',monospace;font-size:.95rem}
.sheet .row{display:flex;gap:10px;margin-top:14px}
.sheet .row .btn{flex:1;justify-content:center}
.err{color:#f87171;font-size:.82rem;margin-top:8px;min-height:1em}
